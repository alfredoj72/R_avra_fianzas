...
#el id_bi (deleg || cmuntrib_dgc || parcela || nordenbifis)
...
#esto orden es muy lenta creo que porque no hay indices definidos en la tabla
orden <- "
...
# Si me quedo con el primer registro que encuentre es más rápido
# orden <- "
#   CREATE TABLE tmp_avra_alquiler.parcelas_supef_tmp AS
#   SELECT DISTINCT ON (refcat_d) *
...
consulta2 <- "SELECT t1.*,
                     t2.idvcat, t2.tip_const4d_14, t2.stotalocal_14, t2.a_ant_bim,
                     t3.sup AS sup_parcela, coorx, coory, num_repeticiones
             FROM tmp_avra_alquiler.fianzas_2022 AS t1
             LEFT JOIN catastro2022.modelo_vivienda_20221011 AS t2
                  ON t1.id_bi = t2.id_bi
...
##########################################################
# comprobar existencia de esquema y tabla antes de crear
# ejecutar correcciones con source
# si no existe ninguna de las 3 tablas crea el esquema y las tablas
if(!dbExistsTable(con, Id(schema = "catastro_gml_2022", table="building")) &
   !dbExistsTable(con, Id(schema = "catastro_gml_2022", table="parcelas")) &
   !dbExistsTable(con, Id(schema = "catastro_gml_2022", table="metadatos"))
) {
  source(paste(getwd(), 
               "/scripts/Crea_estructura_en_postgree.R",sep =""),
         encoding = 'UTF-8')
}
